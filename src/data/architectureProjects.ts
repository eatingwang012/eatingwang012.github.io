// Architecture Projects Data - REPLACE WITH YOUR PROJECTS
export const architectureProjects = [
  {
    id: 3,
    title: "荔枝采摘机器人",
    description: "针对岭南丘陵地带设计的荔枝机器人，从采摘、存储到运输，护送荔枝的一生。",
    imageUrls: [
      "/lychee1.jpg",
      "/lychee2.jpg",
      "/lychee3.jpg",
      "/lychee4.jpg",
      "/lychee5.jpg",
      "/lychee6.jpg",
      "/lychee7.jpg",
      "/lychee8.jpg",
      "/lychee9.jpg",
      "/lychee10.jpg",
      "/lychee11.jpg"
    ],
    tags: ["工业设计", "机械结构", "农业"],
    githubUrl: "https://github.com/zhangsan/cultural-center"
  },
  {
    id: 1,
    title: "行李寄送服务设计",
    description: "瞄准差旅人群退房后行李不便携带、跨区寄存难的核心痛点，提供行李寄送服务。",
    imageUrls: [
      "/hotel1.jpg",
      "/hotel2.jpg",
      "/hotel3.jpg",
      "/hotel4.jpg",
      "/hotel5.jpg",
      "/hotel6.jpg",
      "/hotel7.jpg",
      "/hotel8.jpg",
      "/hotel9.jpg",
      "/hotel10.jpg",
      "/hotel11.jpg",
      "/hotel12.jpg"
    ],
    tags: ["服务设计", "概念设计", "环保理念"],
    githubUrl: "https://github.com/zhangsan/modern-house"
  },
  {
    id: 2,
    title: "荔枝认养系统设计",
    description: "聚焦荔枝零售推出\"线上认养 + 实时果园监控\"，解决产销侧卖果难与辨真难的痛点。",
    imageUrls: [
      "/bee1.jpg",
      "/bee2.jpg",
      "/bee3.jpg",
      "/bee4.jpg",
      "/bee5.jpg",
      "/bee6.jpg",
      "/bee7.jpg",
      "/bee8.jpg",
      "/bee9.jpg",
      "/bee10.jpg",
      "/bee11.jpg",
      "/bee12.jpg"
    ],
    tags: ["系统设计", "服务设计", "农业"],
    githubUrl: "https://github.com/zhangsan/commercial-complex"
  },

  {
    id: 4,
    title: "儿童粤语学习机",
    description: "帮助随迁子女融入粤语环境、兼顾粤语培养与岭南文化认知，满足幼儿早教需求。",
    imageUrls: [
      "/canton1.jpg",
      "/canton2.jpg",
      "/canton3.jpg",
      "/canton4.jpg",
      "/canton5.jpg",
      "/canton6.jpg",
      "/canton7.jpg",
      "/canton8.jpg",
      "/canton9.jpg",
      "/canton10.jpg"
    ],
    tags: ["工业设计", "儿童早教", "文化创意"],
    githubUrl: "https://github.com/zhangsan/eco-park"
  },
  {
    id: 5,
    title: "六神音乐节-品牌策划案",
    description: "为国货品牌注入年轻活力，结合产品和用户，开创新场景。",
    imageUrls: [
      "/1.png",
      "/2.png",
      "/3.png",
      "/4.png",
      "/5.png"
    ],
    tags: ["品牌营销", "活动策划", "方案设计"],
    githubUrl: "https://github.com/zhangsan/eco-park"
  }

];